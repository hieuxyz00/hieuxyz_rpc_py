...
class Client:
    ...
    def __init__(self, options: ClientOptions):
        ...
        self.websocket = DiscordWebSocket(self.token, DiscordWebSocketOptions(
        ...
        self.formatters = {
            ...
            'asset': lambda val, _: f'"{val}" (\x1b[34mhttps://cdn.discordapp.com/avatar-decoration-presets/{val}.png\x1b[0m)',
            'accent_color': lambda val, _: f'{val} (\x1b[33m#{hex(val)[2:].zfill(6).upper()}\x1b[0m)' if val else 'null',
    ...
    async def _send_all_activities(self):
        ...
        potential_activities = await asyncio.gather(*(rpc.build_activity() for rpc in self.rpcs))
 ...
 |_| |_|_|\\___|\\__,_/_/\\_\\\\__, /___|  
  ...
  Developed by: hieuxyz
    ...
    def _format_avatar(self, val, parent):
        ...
        url = f"https://cdn.discordapp.com/avatars/{user_id}/{val}.{ext}" if user_id else ''
    ...
    def _format_banner(self, val, parent):
        ...
        ext = 'gif' if val.startswith('a_') else 'png'
        user_id = parent.get('id')
        url = f"https://cdn.discordapp.com/banners/{user_id}/{val}.{ext}" if user_id else ''
        return f'"{val}" {f"(\x1b[34m{url}\x1b[0m)" if url else ""}'
    ...
    def _format_flags(self, flags: int) -> str:
        ...
        flag_names = []
        if flags & UserFlags.STAFF: flag_names.append('Staff')
        if flags & UserFlags.PARTNER: flag_names.append('Partner')
        if flags & UserFlags.HYPESQUAD: flag_names.append('HypeSquad')
        if flags & UserFlags.BUG_HUNTER_LEVEL_1: flag_names.append('BugHunter I')
        if flags & UserFlags.HYPESQUAD_ONLINE_HOUSE_1: flag_names.append('Bravery')
        if flags & UserFlags.HYPESQUAD_ONLINE_HOUSE_2: flag_names.append('Brilliance')
        if flags & UserFlags.HYPESQUAD_ONLINE_HOUSE_3: flag_names.append('Balance')
        if flags & UserFlags.PREMIUM_EARLY_SUPPORTER: flag_names.append('EarlySupporter')
        if flags & UserFlags.BUG_HUNTER_LEVEL_2: flag_names.append('BugHunter II')
        if flags & UserFlags.VERIFIED_DEVELOPER: flag_names.append('VerifiedDev')
        if flags & UserFlags.CERTIFIED_MODERATOR: flag_names.append('CertifiedMod')
        if flags & UserFlags.ACTIVE_DEVELOPER: flag_names.append('ActiveDev')
                ...
                if value is None:
                    ...
                    if key in self.formatters:
                        ...
                        if isinstance(value, str):
                            ...
                            display_value = '\x1b[32mtrue\x1b[0m' if value else '\x1b[31mfalse\x1b[0m'
