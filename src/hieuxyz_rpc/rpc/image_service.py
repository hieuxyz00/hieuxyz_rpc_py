...
class ImageService:
    ...
    """
    def __init__(self, api_base_url: str = 'https://rpc.hieuxyz.fun'):
        """
    ...
    async def get_external_url(self, url: str) -> Optional[str]:
        """
        ...
        Returns:
            Optional[str]: Asset key resolved or None if failed.
        """
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{self.api_base_url}/image", params={"url": url}) as response:
    ...
    async def upload_image(self, file_path: str, file_name: str) -> Optional[str]:
        """
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(f"{self.api_base_url}/upload", data=data) as response:
    ...
    async def renew_image(self, asset_id: str) -> Optional[str]:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(f"{self.api_base_url}/renew", json={"asset_id": asset_id}) as response:
                    data = await response.json()
                    if data and 'id' in data:
    ...
    async def fetch_application_assets(self, application_id: str) -> List[Dict[str, Any]]:
        """
